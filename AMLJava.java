...
public class AMLJava extends JFrame
{
    ...
    public static boolean pathGen(Cell c, int steps) {
        ...
        else if (maze[cRow-1][cCol] !=  null) dirs.remove("up");
        if (cRow == (height-1)) dirs.remove("down");
        else if (maze[cRow+1][cCol] != null) dirs.remove("down");
        if (cCol == 0) dirs.remove("left");
        else if (maze[cRow][cCol-1] != null) dirs.remove("left");
        if (cCol == (width-1)) dirs.remove("right");
        else if (maze[cRow][cCol+1] != null) dirs.remove("right");
        ...
        if (randDir.equals("up")) {
            maze[cRow-1][cCol] = new Cell(1, cRow, cCol);
            steps--;
            return pathGen(maze[cRow-1][cCol], steps);
        }
        else if (randDir.equals("down")) {
            maze[cRow+1][cCol] = new Cell(1, cRow+1, cCol);
            steps--;
            return pathGen(maze[cRow+1][cCol], steps);
        }
        else if (randDir.equals("left")) {
            maze[cRow][cCol-1] = new Cell(1, cRow, cCol-1);
            steps--;
    ...
    // returns the cell to the right of the bot's current position IF it has been visited before (and exists)
